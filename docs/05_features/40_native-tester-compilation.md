...
As of v1.5.0 native compilation can be performed and a new set of libraries to facilitate native testing and native "scratch pad" compilation. [`infrablockchain-cc`](../03_command-reference/infrablockchain-cc.md), [`infrablockchain-cpp`](../03_command-reference/infrablockchain-cpp.md) and [`infrablockchain-ld`](../03_command-reference/infrablockchain-ld.md) now support building "smart contracts" and unit tests natively for quick tests to help facilitate faster development \(note the default implementations of InfraBlockchain `intrinsics` are currently asserts that state they are unavailable, these are user definable.\)
...
## InfraBlockchain.CDT Native Tester API
